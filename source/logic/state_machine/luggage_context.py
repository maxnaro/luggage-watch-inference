...
from inference.source.logic.state_machine.states import Attended, Unattended, Abandoned, State
from inference.source.logic.helpers.bbox import BBox
from inference.source.constants import MOVEMENT_THRESHOLD_PX, OWNER_RADIUS_PX
...
class LuggageContext:
    ...
    def update(self, luggage_bbox: BBox, person_bboxes: dict[int, BBox]) -> str:
        ...
        if self.last_bbox is not None:
            is_moving = luggage_bbox.distance_to(self.last_bbox) >= MOVEMENT_THRESHOLD_PX
        self.last_bbox = luggage_bbox
        
        is_attended = False
        if self.owner_id is not None and self.owner_id in person_bboxes:
            is_attended = luggage_bbox.distance_to(person_bboxes[self.owner_id]) <= OWNER_RADIUS_PX
    ...
    @staticmethod
    def _get_nearest_person_id(luggage_bbox: BBox, person_bboxes: dict[int, BBox]) -> int | None:
        nearest_id = None
